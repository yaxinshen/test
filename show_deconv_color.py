...
def vis_square(data,i):
    ...
    misc.imsave('img/all%dfilter.png'%(i), data)
...
def deconv48():
    ...
    denet_file = '/home/shenyaxin/color/de_color.prototxt'
    ...
    for i in range(0, len(datas), batch_size):
        ...
        output = net.forward()
        print net.params['conv1'][0].shape
        denet.params['conv1'][0].data[...] = \
        net.params['conv1'][0].data[[51,55,56,57,58,59,60,62,65,66,68,69,70,71,72,73,76,77,78,81,82,83,84,85,87,89,91,94,95]].reshape(denet.params['conv1'][0].data.shape)
        denet.params['conv1t'][0].data[...] = \
        net.params['conv1'][0].data[[51,55,56,57,58,59,60,62,65,66,68,69,70,71,72,73,76,77,78,81,82,83,84,85,87,89,91,94,95]].reshape(denet.params['conv1t'][0].data.shape)
        ...
        #misc.imsave('img/select%d1.png'%(i),imga)
        misc.imsave('img/select%d2.png'%(i),imgb)
...
def deconv_each():
    ...
    for i in range(0, len(datas), batch_size):
        ...
        for step in range(0,96):
            denet.params['conv1'][0].data[...] = net.params['conv1'][0].data[step].reshape(denet.params['conv1'][0].data.shape)
            denet.params['conv1t'][0].data[...] = net.params['conv1'][0].data[step].reshape(denet.params['conv1t'][0].data.shape)
            denet.blobs["data"].reshape(bs,3,224,224)
            denet.blobs["data"].data[...] =  imgs
            deout = denet.forward()
            img_t = denet.blobs['conv1t'].data.copy()
